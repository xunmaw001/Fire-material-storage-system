    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="仓库名称" prop="cangkumingcheng">
          <el-input v-model="ruleForm.cangkumingcheng" 
              placeholder="仓库名称" clearable  :readonly="ro.cangkumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="仓库名称" prop="cangkumingcheng">
              <el-input v-model="ruleForm.cangkumingcheng" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="仓库类型" prop="cangkuleixing">
          <el-select :disabled="ro.cangkuleixing" v-model="ruleForm.cangkuleixing" placeholder="请选择仓库类型">
            <el-option
                v-for="(item,index) in cangkuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="仓库类型" prop="cangkuleixing">
	      <el-input v-model="ruleForm.cangkuleixing"
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.cangkumingcheng = true;
          ...
          if(o=='cangkuleixing'){
            this.ruleForm.cangkuleixing = obj[o];
	    this.ro.cangkuleixing = true;
            continue;
          }
          if(o=='cangkuweizhi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.cangkuleixingOptions = "灭火类,救助类,探测类".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cangku/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cangkuCrossAddOrUpdateFlag = false;
						 this.parent.search();
