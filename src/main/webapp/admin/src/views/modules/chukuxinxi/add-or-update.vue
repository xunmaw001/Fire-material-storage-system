<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.chukuxinxi" label="出库信息" prop="chukuxinxi">
          <file-upload
          tip="点击上传出库信息"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.chukuxinxi?ruleForm.chukuxinxi:''"
          @change="chukuxinxiUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.chukuxinxi" label="出库信息" prop="chukuxinxi">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.chukuxinxi)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='chukuxinxi'){
            this.ruleForm.chukuxinxi = obj[o];
	    this.ro.chukuxinxi = true;
            continue;
          }
        }
      }
	...
	if(this.ruleForm.chukuxinxi!=null) {
		this.ruleForm.chukuxinxi = this.ruleForm.chukuxinxi.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chukuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chukuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    chukuxinxiUploadChange(fileUrls) {
	this.ruleForm.chukuxinxi = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
