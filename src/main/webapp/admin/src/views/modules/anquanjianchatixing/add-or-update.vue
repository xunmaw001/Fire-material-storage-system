    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="内容详情" prop="neirongxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.neirongxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.neirongxiangqing" label="内容详情" prop="neirongxiangqing">
                    <span v-html="ruleForm.neirongxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fengmian = true;
          ...
          if(o=='neirongxiangqing'){
            this.ruleForm.neirongxiangqing = obj[o];
	    this.ro.neirongxiangqing = true;
            continue;
          }
          if(o=='fabushijian'){
    ...
    info(id) {
      this.$http({
        url: `anquanjianchatixing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.neirongxiangqing = this.ruleForm.neirongxiangqing.replace(reg,'../../../ssmr46x1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `anquanjianchatixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.anquanjianchatixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
