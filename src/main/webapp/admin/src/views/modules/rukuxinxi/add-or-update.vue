<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.rukuxinxi" label="入库信息" prop="rukuxinxi">
          <file-upload
          tip="点击上传入库信息"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.rukuxinxi?ruleForm.rukuxinxi:''"
          @change="rukuxinxiUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.rukuxinxi" label="入库信息" prop="rukuxinxi">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.rukuxinxi)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='rukuxinxi'){
            this.ruleForm.rukuxinxi = obj[o];
	    this.ro.rukuxinxi = true;
            continue;
          }
        }
      }
	...
	if(this.ruleForm.rukuxinxi!=null) {
		this.ruleForm.rukuxinxi = this.ruleForm.rukuxinxi.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    rukuxinxiUploadChange(fileUrls) {
	this.ruleForm.rukuxinxi = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
